...
If our line equation is {math}`y = mx + b`, each deviation {math}`\delta y_i` equals the difference between measured y value and the corresponding point on the line:
...
{math}` \frac{\partial M}{\partial m} = 0` and {math}`\frac{\partial M}{\partial b} = 0`
