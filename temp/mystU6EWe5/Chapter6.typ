...
#show: template.with(
  title: "Evaluating Experimental Results",
  date: datetime(
    year: 2025,
    month: 1,
    day: 1,
...
If your gravitational acceleration measurement is 9.60 $plus.minus$ 0.30 m/s², recognize that the uncertainty is as significant as the central value. The 3% uncertainty might reflect equipment limitations rather than experimental shortcomings. Textbook values often appear without context about the sophisticated methods and equipment used to obtain them. Your aim should be honest, objective reporting of results with appropriate uncertainty limits—not perfect reproduction of established values in limited laboratory time.
...
For pendulum length measurements using a meter stick, you've likely determined intervals within which you're confident your true values lie. Your results would appear as a set of values in the form: value $plus.minus$ uncertainty.

Similarly, if you've counted oscillations and timed them with a stopwatch, you might express time measurements with their uncertainty ranges. However, the oscillation period T (your actual variable of interest) must be calculated from these measurements. If you counted 15 oscillations that took 18.4 $plus.minus$ 0.2 seconds, the period for a single oscillation would be:

(1/15)(18.4 $plus.minus$ 0.2) = 1.227 $plus.minus$ 0.013 seconds
...
$ R = v a l u e ± u n c e r t a i n t y $
...
$ v a l u e (+ u n c e r t a i n t y ₁ \/ -u n c e r t a i n t y ₂) $
visual graph judgment rarely justifies such precision. If identifying a clear "best" line proves genuinely difficult, you can simply delineate the edges of the value band (lines ED and CF in Figure 6.3), calculate maximum and minimum slopes, and express your experimental result as the interval between these slopes, or as their average $plus.minus$ half their difference.
...
If your model predicts an intercept from which you hope to determine some quantity, the intersection of your three lines with the relevant axis directly provides that intercept as: value $plus.minus$ uncertainty.
...
If our line equation is y = mx + b, each deviation $delta$yᵢ equals the difference between measured y value and the corresponding point on the line:

$ delta y ᵢ = y ᵢ -(m x ᵢ + b) $
...
$ m = [ N ∑ (x ᵢ y ᵢ) -∑ x ᵢ ∑ y ᵢ ] \/ [ N ∑ x ᵢ ² -(∑ x ᵢ) ² ] $
$ b = [ ∑ x ᵢ ² ∑ y ᵢ -∑ x ᵢ ∑ (x ᵢ y ᵢ) ] \/ [ N ∑ x ᵢ ² -(∑ x ᵢ) ² ] $
...
The best quantity to report is standard deviation of the mean, which has recognizable numerical significance. Sometimes standard deviation itself is quoted. Always specify sample size so $sigma$ estimate reliability can be judged.
...
No unique answer exists for significant figure questions, but generally avoid retaining figures beyond the first uncertain digit. For example, 5.4387 $plus.minus$ 0.2 should be reported as 5.4 $plus.minus$ 0.2, since uncertainty in the tenths position makes subsequent digits meaningless. If uncertainty is known more precisely, retaining one additional figure might be justified—if uncertainty were 0.15, reporting 5.44 $plus.minus$ 0.15 would be valid.

When reporting percentage precision, significant figures are automatically implied. A measurement reported as 527.64182 $plus.minus$ 1% implies absolute uncertainty of 5.2764. However, since precision is quoted to just one significant figure (1%, not 1.000%), the uncertainty itself warrants only one significant figure. Calling it 5 implies the tens digit in the original number is uncertain by 5, making subsequent digits meaningless. The measurement should be quoted as 528 $plus.minus$ 5 or 528 $plus.minus$ 1%.
...
Finally, always ensure answer and uncertainty expressions are consistent—neither "16.2485 $plus.minus$ 0.5" nor "4.3 $plus.minus$ 0.0002" represents good practice.
