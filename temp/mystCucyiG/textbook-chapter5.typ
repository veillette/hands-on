...
$ t = 0. 4515 x^(1 \/ 2) quad "(in meters and seconds)" $
To represent this in linear form:

$ "vertical variable" = "slope"times "horizontal variable"+ "intercept" $
We might choose:
...
You might consider plotting Q versus ($pi$a⁴/8ℓ)P to get a slope of 1/$eta$, but this introduces unnecessary complications. Each pressure measurement would require multiplication by $pi$a⁴/8ℓ, and uncertainties in a and ℓ would artificially inflate the uncertainty in your compound quantity.

A better approach: plot Q versus P directly, using $pi$a⁴/8$eta$ℓ as the slope. This simplifies both your plotting process and uncertainty analysis. After determining the slope, calculate $eta$ using:

$ eta = frac(pi a^4, 8 ell times "slope") $
...
$ T^2 = frac(4 pi^2 (h^2 + k^2), g h) $
Multiplying both sides by h:

$ T^2 h = frac(4 pi^2 (h^2 + k^2), g) $
Rearranging to isolate h²:

$ h^2 = frac(g, 4 pi^2) T^2 h -k^2 $
...
If you can measure length with $plus.minus$1mm uncertainty, the minimum acceptable length measurement would be:

$ frac(0. 001 "m", ell) = 0. 01 \/ $

$ ell = 0. 1 "m" $
Similarly, if timing uncertainty is $plus.minus$0.2s, and period measurement requires 0.5% precision (for 1% in T²), the minimum timing interval would be:
...
$ g = ("dimensionless constant") times frac("length", "period"^2) $
This approach can't determine dimensionless constants (like $pi$), but it reveals functional relationships between variables.
...
$ L T^(-1) = (M L T^(-2))^a (M L^(-1))^b = M^(a + b) L^(a -b) T^(-2 a) $
Matching powers:
...
Solving gives a=$1/2$, b=-$1/2$, yielding:

$ v = ("dimensionless constant") times sqrt(frac(T, m)) $
This powerful technique becomes more complex with many variables. When analyzing fluid flow rate (Q) through a tube with pressure difference (P), radius (r), length (ℓ), and viscosity ($eta$), we get:

$ Q prop frac(P, eta) times r^3 times (frac(r, ell))^b $
We can determine some relationships (Q contains P/$eta$ and r³) but not all (the exponent b remains undetermined). Even this partial solution provides valuable experimental guidance.
...
+ A projectile is launched with initial velocity v at angle $alpha$ to the horizontal. Its range may depend on the projectile's mass, initial velocity, launch angle, and gravitational acceleration. Determine the functional form of this relationship through dimensional analysis.
...
where T and $alpha$ are measurable, and ℓ is fixed and known. Determine g.
+ The deflection of a cantilever beam follows:

$ d = frac(4 W ell^3, Y a b^3) $
...
where h and R are measurable, and $rho$ and g are known constants. Determine surface tension $sigma$.
...
where ℓ and $Delta$T are measurable, and ℓ₀ is unknown but constant. Determine coefficient of expansion $alpha$.
+ Snell's law of refraction states:

$ n_1 sin theta_1 = n_2 sin theta_2 $
where $theta$₁ and $theta$₂ are measurable angles, and n₁ is a known constant. Determine refractive index n₂.
...
$ F = frac(mu_0, 4 pi) frac(i_1 i_2 ell^2, r^2) $
where F, i₁, i₂, and r are measurable quantities, while $mu$₀ and ℓ are constants. Describe how to verify this relationship.
...
where Z and $omega$ are measurable. Determine resistance R and capacitance C.
