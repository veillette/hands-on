...
If our line equation is $y = m x + b$, each deviation $delta y_i$ equals the difference between measured y value and the corresponding point on the line:

$ delta y_i = y_i -(m x_i + b) $
The least squares criterion seeks to minimize:

$ sum_i [ y_i -(m x_i + b) ]^2 = chi $
with conditions:

$frac(diff M, diff m) = 0$ and $frac(diff M, diff b) = 0$
